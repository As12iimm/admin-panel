...
function showApp() { show('loginView', false); show('mfaView', false); show('appView', true); const w = el('welcomeLine'); if (w) w.textContent = `${currentUser?.name || 'User'} (${currentUser?.role || ''})`; applyPermissionGuards(); }
...
async function login() {
  try {
    ...
    const data = await fetch('/api/auth/login', {
      ...
      body: JSON.stringify({ email: emailValue, password: passwordValue, deviceName: 'web' })
    }).then((r) => r.json());

    if (data.mfaRequired) {
      mfaChallengeToken = data.challengeToken;
      const hint = el('mfaHint');
      if (hint) hint.textContent = `OTP sent. (Dev OTP: ${data.devOtp})`;
      showMfa();
      return;
    }
...
async function verifyMfa() {
  try {
    const codeValue = (el('mfaCode')?.value || '').trim();
    const data = await fetch('/api/auth/mfa/verify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ challengeToken: mfaChallengeToken, code: codeValue, deviceName: 'web' })
    }).then((r) => r.json());

    if (!data.accessToken) {
      const msg = el('mfaMsg');
      if (msg) msg.textContent = data.message || 'Invalid OTP';
      return;
    }
    ...
    const msg = el('mfaMsg');
    if (msg) msg.textContent = 'OTP verification failed';
  }
}
...
async function requestPasswordReset() {
  try {
    ...
    const msg = el('loginMsg');
    if (msg) msg.textContent = `Reset requested. Dev token: ${data.devResetToken || '-'}`;
  } catch {
    const msg = el('loginMsg');
    if (msg) msg.textContent = 'Password reset request failed';
  }
}
